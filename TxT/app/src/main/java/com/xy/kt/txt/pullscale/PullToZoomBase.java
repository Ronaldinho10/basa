...
public abstract class PullToZoomBase<T extends View> extends LinearLayout implements IPullToZoom<T> {
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                if (isReadyForPullStart()) {
                    Log.i("onInterceptTouchEvent  ","ACTION_MOVE   isReadyForPullStart ");
                    ...
                    if (absDiff > mTouchSlop && absDiff > Math.abs(oppositeDiff)) {
                        if (diff >= 1f && isReadyForPullStart()) {
                            ...
                            Log.i("onInterceptTouchEvent  ","mIsBeingDragged = true ");
                        }
                    }
                }
                break;
            ...
            case MotionEvent.ACTION_DOWN: {
                if (isReadyForPullStart()) {
                    Log.i("onInterceptTouchEvent  ","ACTION_DOWN   isReadyForPullStart ");
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN: {
                Log.i("onTouchEvent  ","ACTION_DOWN  not isReadyForPullStart ");
                if (isReadyForPullStart()) {
                    Log.i("onTouchEvent  ","ACTION_DOWN   isReadyForPullStart ");
                    mLastMotionY = mInitialMotionY = event.getY();
                    mLastMotionX = mInitialMotionX = event.getX();
                    return true;
                }
